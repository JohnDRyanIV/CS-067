...
public class Main extends Application 
{
    ...
    public void start(Stage stage) 
    {
        ...
        Button descriptionSortBtn = new Button("Description");
        Button nameSortBtn = new Button("Name");
        Button completionSortBtn = new Button("Completion");
        
        descriptionSortBtn.setOnAction(new TaskSorter());
        nameSortBtn.setOnAction(new TaskSorter());
        completionSortBtn.setOnAction(new TaskSorter());
        
        Label sort = new Label("Sort By:");
        

        
        HBox sortArea = new HBox(5, descriptionSortBtn, nameSortBtn, completionSortBtn);
        VBox sortVert = new VBox(5, sort, sortArea);
        
        // Create a root container that holds both the task list and the input area
        VBox root = new VBox(scrollableCheckBoxDisplay,inputArea, sortVert);
    ...
    /**
     * Sorts tasks in currentList based on text in the sorting button, then refreshes task display.
     */
    private class TaskSorter implements EventHandler<ActionEvent>
    {

		@Override
		public void handle(ActionEvent event) {
			Button btn = (Button) event.getSource();
			if(btn.getText().equalsIgnoreCase("description")) {	// sort by description
				Collections.sort(currentList, new DescriptionComparator());
			}
			else if (btn.getText().equalsIgnoreCase("name")) {	// sort by name
				Collections.sort(currentList, new NameComparator());

			}
			else if (btn.getText().equalsIgnoreCase("completion")) {	// sort by completion status
				Collections.sort(currentList, new CompleteComparator());
			}
			else {
				System.out.println("Invalid button selection");
			}
